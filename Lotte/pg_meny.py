"""
Har modifisert dette programmet litt. Har endret så nedtrekkslistene tegnes bortover på siden og ikke alt ned i en kolonne.
"""
...
class Nedtrekksliste:
  ...
  def alternativRamme(self, i):
    #regner ut hvor mange elementer det er plass til nedover i vinduet
    #trekker fra to for selve listeknappen + litt buffer øverst og nederst
    antall_elementer = round(VINDU_HOYDE / self.hoyde - 2)

    kolonne = i // antall_elementer
    rad = i % antall_elementer
    return pg.Rect(
      self.xPosisjon + kolonne * (self.bredde),
      self.yPosisjon + (rad + 1) * self.hoyde,
      self.bredde,
      self.hoyde
    )
